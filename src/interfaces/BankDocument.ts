    ...
    if (comment) {
      ...
      requests.forEach((request: string) => {
        ...
        if (requestNumber) {
          ...
          let valueInt =
            value && !Number.isNaN(+valueFloat)
              ? valueFloat
              : this.income || this.outcome
          //   client = client ? client : this.client
          client = comment

          let requestType: BankRequestType = this.type
            ? this.type
            : this.income
            ? 'income'
            : 'outcome'
          // console.log('requestNumber',requestNumber)
          const correctRequestNumber = requestNumber.match(
            /[а-я,А-Я]{3}-[0-9]{1,2}($|\/[0-9]{1,6})/g
          )[0]
          this.requestsSum += valueInt
          const bankRequest = new BankRequest(
            correctRequestNumber,
            valueInt,
            requestType,
            this.id,
            client,
      ...
      if (this.requests.length > 1) {
        this.error =
          Math.round((this.requestsSum + Number.EPSILON) * 100) / 100 ===
          (this.income || this.outcome)
            ? false
            : true
      } else {
        this.error =
          this.requests[0].value === (this.income || this.outcome)
            ? false
            : true
      }
    }
